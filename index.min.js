...
var domApp, domTime, domDate, domCnDate, domFestival, domWeather;
var urlQuery;
var config; // Made global for oled.js
var scriptLoadDateNormalized;
var exactScriptLoadTimeGlobal;
...
function render() {
    if (!domApp || typeof calendar === 'undefined' || !config || typeof exactScriptLoadTimeGlobal === 'undefined') { // Check for new global
        console_log_on_screen("Render: DOM, calendar, config, or exactLoadTime not ready.");
        return;
    }
    ...
    try {
        ...
        var msSinceScriptLoad = actualCurrentTime.getTime() - exactScriptLoadTimeGlobal.getTime();
        
        var dailyAccumulatedOffsetms = 0;
        if (!isNaN(config.ofs) && config.ofs !== 0) {
            dailyAccumulatedOffsetms = (msSinceScriptLoad / millisecondsPerDay) * config.ofs;
        }
//        if (config.ofs !== 0) {
//            var logMessage = "ofs: " + config.ofs + "ms/day, MS_SinceLoad: " + msSinceScriptLoad + ", AccumOffset: " + dailyAccumulatedOffsetms + "ms";
...
//        }
        ...
        var cnDateText = lunar.IMonthCn + lunar.IDayCn;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var S_exactScriptLoadTime = new Date(); 
    scriptLoadDateNormalized = new Date(S_exactScriptLoadTime.getFullYear(), S_exactScriptLoadTime.getMonth(), S_exactScriptLoadTime.getDate());
    ...
    domWeather = document.querySelector(".weather");
    ...
    exactScriptLoadTimeGlobal = S_exactScriptLoadTime;
    console_log_on_screen("Config: tz=" + config.timezone + ", fs=" + config.fontSize + ", r=" + config.rotate + ", ofs=" + config.ofs + ", lang=" + config.lang);

    domTime.style.fontSize = (config.fontSize * 1.3) + "rem";
    domDate.style.fontSize = (config.fontSize * 0.433) + "rem";
    domCnDate.style.fontSize = (config.fontSize * 0.433) + "rem";
    domFestival.style.fontSize = (config.fontSize / 4) + "rem";
    if (domWeather) domWeather.style.fontSize = (config.fontSize * 0.433) + "rem";
    ...
    if (typeof weather !== 'undefined' && domWeather) {
        try {
            ...
            console_log_on_screen("Error initializing weather: " + e.message);
        }
    }
